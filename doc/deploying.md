...
of Nullchan in the wild, and any configuration options that you should be aware of
...
be relevant to any Nullchan instance.


Production environment requirements
-----------------------------------

Besides obviously Nullchan, your production environment should have a couple
other pieces of software installed:

* A SQL database. PostgreSQL is the primary supported platform, but SQLite works as
  well, and will probably suffice if you're fine with hosting Nullchan and the database
...
* An S3 instance or S3-compatible object store. Minio is known to work and is the
  S3-compatible store used when using the sample `docker-compose.yml` in Nullchan's
...
* A keyvalue/pubsub store. Redis is recommended for production deployments, but Nullchan
  comes with a stub implementation that can work if Redis is unviable.
* Docker is recommended for deploying Nullchan itself, since it removes the need
  to install and manage prerequisites for Nullchan in addition to making updating
...
* A reverse proxy is also recommended, but is not strictly necessary; the default uWSGI
  configuration bundled with Nullchan in `deploy-configs` assumes a WSGI-capable reverse
  ...
  `docker-compose.yml` - should work with no modification if nginx and Nullchan live
  on the same machine.


Configuring Nullchan
--------------------

Nullchan can be configured for production entirely by modifying the files present in
the `deploy-configs` directory; you have the option of modifying the files from an
archive of Nullchan's source and then building and deploying a Docker image from that,
or you can copy the directory out, make the necessary modifications for your environment, and
then mount your directory to `/nullchan/deploy-configs` when running Nullchan under
Docker; the latter is the recommended approach and the one Futatsu takes. In particular,
you should definitely modify `nullchan.cfg`, `bootstrap-config.json`, and `index-greeting.html`,
but `uwsgi.ini` is available if you need to modify the way the uWSGI application server
hosts Nullchan. The specifics of uWSGI's configuration settings are outside the scope of this
document, but the default supplied with Nullchan should be sufficient for most deployments.

`nullchan.cfg` is the file that Nullchan refers to for database/S3 endpoint info as well as
...
* `INSTANCE_NAME` - this string controls the name of the deployed Nullchan instance, which
...
* `STORAGE_PROVIDER` - this variable indicates whether static assets are stored in an S3
  ...
  Nullchan itself. For S3, this should be set to `"S3"`; for the filesystem backend,
  choose `"FOLDER"`. In a production environment, an S3 store is almost certainly the better
  option.
* `S3_ENDPOINT` - a string storing the endpoint URL for your S3 store that Nullchan should contact.
...
* `S3_UUID_PREFIX` - Amazon, among S3 providers, requires that bucket names be globally unique across
  their service. This is an optional string that Nullchan will prepend to the names of the buckets
...
* `CDN_REWRITE` - this is an optional string that, if present, will be used as a template by the
  ...
  supplied by Nullchan are: `ENDPOINT`, `BUCKET_UUID`, `BUCKET`, and `PATH`. `PATH` is the path to the
  resource (and does not begin with a `/`), while `BUCKET_UUID` is equal to `S3_UUID_PREFIX`, if it is
  present.
* `STORE_PROVIDER` - this is a string indicating which keystore/pubsub provider Nullchan should target.
  Valid values are `"REDIS"` for Redis, or `"INTERNAL"` for Nullchan's built-in substitute.
* `REDIS_HOST` - this is an optional string that contains the URL of the Redis database if `STORE_PROVIDER`
  is set to `"REDIS"`.
* `SERVE_STATIC` - this is a boolean that indicates whether Nullchan should serve up CSS/JS/other
...
* `SERVE_REST` - if `True`, REST endpoints will be served by Nullchan. Note that ideally, the REST API for
  your Nullchan installation is protected with HTTPS, since Nullchan uses HTTP Basic for authentication and
  authorization.
* `RENDERER_HOST` - this is a required string indicating the URL of the Nullchan frontend. For a production
...
* `DEFAULT_THEME` - this is an optional string representing the name of the theme Nullchan should use if
...
* `THEME_LIST` - this is an optional list of strings representing the list of installed themes available.
  If not present, Nullchan assumes the default themes normally bundled with Nullchan are available. 
* `MAX_CONTENT_LENGTH` - this is an integer detailing the largest attachment Nullchan should accept
  in bytes. Note that your reverse proxy should be set up to not deny any requests smaller than this;
  if you're copying or following along with the `nginx.conf` bundled with Nullchan under the `deploy-configs`
  directory, you should be okay.
* `CACHE_VIEW_RATIO` - an optional number that Nullchan will check when caching threads. If set, Nullchan will cache
  a pre-rendered copy of a thread until the real view count divided by the view count in the cached render is greater
  than this number. If not set, Nullchan will not attempt to fetch any fully-rendered threads from the cache. Set this
  if site performance proves to be an issue; a good initial value is `1.15`.

`bootstrap-config.json` is the file used upon bootstrapping Nullchan for the first time to set up any boards,
...
Next is `boards`, which is an array containing a list of objects representing each board that Nullchan should
...
* `name` - this is simply a string containing the name of the board in question. Ideally, each name should be
  around 6-7 characters max to fit best with Nullchan's UI. You can modify the name of the board from the admin
...
* `tag` - the name of the tag. By convention, Nullchan tags are all lowercase with no spaces, though spaces and
...
* `bgstyle` - the CSS class given to the background of threads with this tag as the first tag in the catalog
  and firehose. This can be a Bootstrap class or a custom CSS class if you have modified Nullchan's CSS.
...
Running Nullchan's bootstrap script should be all that's necessary to set up your
...
```
docker -v /path/to/production/deploy-configs:/nullchan/deploy-configs run dangerontheranger/nullchan bootstrap
```
...
after Nullchan has bootstrapped your buckets:
...
Running Nullchan is highly dependent on how you've organized things in your particular production
...
bringing up the Nullchan container for the first time:

	mkdir uploads && mkdir uploads/thumbs
	chmod -R 757 uploads

Taken from [issue #67](https://github.com/DangerOnTheRanger/nullchan/issues/67#issuecomment-477000278). 
